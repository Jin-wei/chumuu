...
app.controller("qrcodeController", ['$rootScope','$scope','$routeParams','$mpAjax','$location','$mp_json','$cookieStore','$q',function($rootScope,$scope,$routeParams,$mpAjax,$location,$mp_json,$cookieStore,$q) {
    var L = $rootScope.L;
    $scope.qrcodes = [];
    ...
    function loadQrcodesPromise(){
        ...
        if ($scope.searchQrcode.length>0){
            search+='&code='+$scope.searchQrcode;
        }
        if ($scope.searchBizName.length>0){
            search+='&biz_name='+$scope.searchBizName;
        }

        $mpAjax.get('/admin/'+ $rootScope.adminId +'/usedQrCodes'+search).then(function(data){
